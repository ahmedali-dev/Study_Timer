...
def main(page: ft.Page):
    ...
    content_area = ft.Container(content=ft.Text("loading..."))
        ...
        while sessionFound <= session and running:
            ...
            while total_study_seconds > 0 and running:
                if not paused:
                    ...
                    update_timer_display("Study: "+str(sessionFound), mins, sec)
                    time.sleep(1)
            ...
            while total_break_seconds > 0 and running:
                if not paused:
                    ...
                    update_timer_display("Break: "+str(sessionFound), mins, sec)
    ...
    def startTimer():
        ...
        # pygame.mixer.music.play()
        savedTimer()
        threading.Thread(target=countdown, args=(session,study_time,break_time), daemon=True).start()
    ...
    def pauseTimer(page):
        ...
        if paused:
            plyer.notification.notify(
                title="Timer Paused", message="Timer is Paused", timeout=3
            )
        page.update()
